...
const UpdateProductForm = ({setModified,setIsediting,isEditing}) => {
  ...
  const [count,setCount]=useState(1)
    ...
    const getProductDetails=useCallback(async()=>{
        try {
            const response = await axios.get(`${BASE_API_ENDPOINT}products/getproductdetails/${isEditing}`)
  ...
  const handleUpdateProduct=async()=>{
    try{
      if(productname!=='' && price!==0 && description!=='' && image ){
        ...
        await axios.put(`${BASE_API_ENDPOINT}products/updateproduct`,{
         productname,
         image,
         category,
         description,
  ...
  return (
    <div className='h-full w-full flex flex-col justify-center items-center p-4 space-y-10 bg-slate-200'>
      <Toaster/>
      {
                imageSettingBoxOpen && 
          ...
          <div className=" w-[90%] flex flex-col items-center space-y-3">
                ...
                <div className="w-full flex items-center space-x-6">
                    ...
                    <p className="text-xs">Category</p>
                     <select onChange={handleCategorySelect} value={category} className="border focus:outline-none">
                    ...
                    <p className="text-xs">Availability</p>
                     <select onChange={handleAvailabilitySelect} value={availability} className="border focus:outline-none">
          ...
          <button onClick={()=>setIsediting(null)} className={`   w-[40%] h-10 text-white bg-red-900 rounded-lg `}>Cancel</button>
          <button onClick={handleUpdateProduct}  className={`   w-[40%] h-10 text-white bg-blue-900 rounded-lg `}>{loading ? 'Loading...':'Update Product'}</button>
          </div>
    </div>
  )
}
